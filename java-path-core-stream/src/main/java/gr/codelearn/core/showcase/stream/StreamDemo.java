...
public class StreamDemo {
	...
	public static void streamIntermediateActions() {
		...
		List<String> list = Stream.of("Monkey", "Lion", "Giraffe", "Lemur").filter(s -> s.startsWith("L")).map(
				String::toUpperCase).sorted().toList();
		...
		logger.info(LINE_DELIMITER);
		sortedCustomStream.sorted(Comparator.comparing(String::length).thenComparing(String::valueOf).reversed())
						  .forEach(logger::info);
		logger.info("");
		...
		logger.info(LINE_DELIMITER);
		List<String> names = createSampleNameList(10);
		names.forEach(logger::info);

		Stream<String> flatMapStream = names.stream();
	...
	public static void streamFinalActions() {
		...
		collectMapStream
				...
				.collect(toMap(Function.identity(), s -> (int) s.chars().distinct().count()))
		...
		Map<Character, Long> groupingByDownstreamMap = collectGroupingByDownstreamCollector.distinct().sorted().collect(
				Collectors.groupingBy(s -> s.charAt(0), counting()));
